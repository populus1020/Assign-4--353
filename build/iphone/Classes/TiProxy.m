...
//Rare exceptions to indicate a bug in the _assign4_353 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_assign4_353 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_assign4_353]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.assign4_353.module"
